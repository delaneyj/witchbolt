# Introduction to witchbolt command line

`witchbolt` provides a command line utility for inspecting and manipulating witchbolt database files. To install witchbolt command-line please refer [here](https://github.com/etcd-io/witchbolt#installing)

**Note**: [etcd](https://github.com/etcd-io/etcd) uses witchbolt as its backend storage engine. In this document, we take etcd as an example to demonstrate the usage of witchbolt commands. Refer to [install etcd](https://etcd.io/docs/v3.5/install/) for installing etcd.
...
1. Start a single member etcd cluster with this command below:
    ...
    └── member
        ├── snap
        │   └── db // this is witchbolt database file
        └── wal
...
3. Stop the etcd instance. Note a witchbolt database file can only be opened by one read-write process, because it is exclusively locked when opened.

## Usage

- `witchbolt command [arguments]`
...
- help will print information about that command
  ...
  The commands are:
      ...
      version     prints the current version of witchbolt
      bench       run synthetic benchmark against witchbolt
      buckets     print a list of buckets
      check       verifies integrity of witchbolt database
      compact     copies a witchbolt database, compacting it in the process
      ...
      surgery     perform surgery on witchbolt database
  ```

- you can use `help` with any command: `witchbolt [command] -h` for more information about command.

## Analyse witchbolt database with witchbolt command line

### version

- `version` print the current version information of witchbolt command-line.
- usage:
  `witchbolt version`
...
- usage:
  `witchbolt info [path to the witchbolt database]`

    Example:

    ```bash
    $witchbolt info ~/default.etcd/member/snap/db
    Page Size: 4096
    ```
...
- `buckets` print a list of buckets of Bbolt database is currently having. Find more information on buckets [here](https://github.com/etcd-io/witchbolt#using-buckets)
- usage:
  `witchbolt buckets [path to the witchbolt database]`

    Example:

    ```bash
    $witchbolt buckets ~/default.etcd/member/snap/db
    alarm
    auth
    authRoles
    authUsers
  ...
  - It means when you start an etcd, it creates these `10` buckets using witchbolt database.
...
- usage:
  `witchbolt check [path to the witchbolt database]`

    Example:

    ```bash
    $witchbolt check ~/default.etcd/member/snap/db
    ok
    ```
...
- To gather essential statistics about the witchbolt database: `stats` performs an extensive search of the database to track every page reference. It starts at the current meta page and recursively iterates through every accessible bucket.
- usage:
  `witchbolt stats [path to the witchbolt database]`

  Example:

  ```bash
  $witchbolt stats ~/default.etcd/member/snap/db
...
- Usage: `witchbolt inspect [path to the witchbolt database]`

  Example:
```bash
$ ./witchbolt inspect ~/default.etcd/member/snap/db
{
    "name": "root",
    "keyN": 0,
    "buckets": [
...
- usage:
  `witchbolt pages [path to the witchbolt database]`

  Example:

  ```bash
  $witchbolt pages ~/default.etcd/member/snap/db
...
- usage:
  ...
  ```bash
  bolt page [path to the witchbolt database] pageid [pageid...]
  or: bolt page --all [path to the witchbolt database]
  ...
  ```bash
  $witchbolt page ~/default.etcd/member/snap/db 3
  Page ID:    3
  Page Type:  leaf
...
- usage:
  ...
  ```bash
  bolt page-item [options] [path to the witchbolt database] <pageId> <itemId>
  Additional options include:
  ...
  ```bash
  $witchbolt page-item --key-only ~/default.etcd/member/snap/db 3 7
  "members"
  ```
...
- usage:
  `bolt dump [path to the witchbolt database] [pageid...]`
...
- usage:
  ...
  ```bash
  bolt keys [path to the witchbolt database] [BucketName]
  ...
  ```bash
  $witchbolt keys ~/default.etcd/member/snap/db meta
  confState
  consistent_index
  term
  ```
  ...
  ```bash
  $witchbolt keys ~/default.etcd/member/snap/db members
  8e9e05c52164694d
  ```
...
- usage:
  ...
  ```bash
  bolt get [path to the witchbolt database] [BucketName] [Key]
  ...
  ```bash
  $witchbolt get --format=hex ~/default.etcd/member/snap/db meta term
  0000000000000004
  ```
  ...
  ```bash
  $witchbolt get ~/default.etcd/member/snap/db members 8e9e05c52164694d
...
- usage:
  ...
  ```bash
  witchbolt compact [options] -o [Destination Path] [Source Path]
  ...
  ```bash
  $witchbolt compact -o ~/db.compact ~/default.etcd/member/snap/db
...
- run synthetic benchmark against witchbolt database.
- usage:
    ...
    ```bash
    $witchbolt bench ~/default.etcd/member/snap/db -batch-size 400 -key-size 16
